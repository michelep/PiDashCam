...
class GY80Poller(threading.Thread):
  ...
  def run(self):
    while self.running:
	Config.gy80.update()
    	w, x, y, z = Config.gy80._current_hybrid_orientation_q

        yaw, pitch, roll = quaternion_to_euler_angles(w, x, y, z)
        Config.log.debug("Gyroscope/Accl/Comp q (%0.2f, %0.2f, %0.2f, %0.2f) yaw %0.1f, pitch %0.2f, roll %0.1f (degrees)" % (w, x, y, z,
                                                                    yaw   * 180.0 / pi,
                                                                    pitch * 180.0 / pi,
                                                                    roll  * 180.0 / pi))
	sleep(5)
